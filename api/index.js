...
<body oncontextmenu="return false;">
    ...
    <script>
        ...
        async function loadVideo() {
            ...
            try {
                ...
                if (data.type === 'embed') {
                    ...
                    // CRITICAL: Aggressive preloading for zero buffering
                    vid.preload = 'auto';
                    
                    // Force browser to buffer aggressively
                    vid.setAttribute('preload', 'auto');
                    vid.setAttribute('autoBuffer', 'true');
                    
                    // Set source and start loading immediately
                    vid.src = proxyUrl;
        ...
        function initControls() {
            ...
            // Optimize video for smooth playback with zero buffering
            vid.volume = 1.0;
            
            // CRITICAL: Force aggressive buffering
            if (vid.buffered) {
                // Monitor buffering and hide spinner aggressively
                const checkBuffer = setInterval(() => {
                    if (vid.buffered.length > 0 && vid.readyState >= 2) {
            ...
            vid.addEventListener('loadedmetadata', () => {
                ...
                // Start buffering immediately after metadata loads
                if (vid.readyState < 2) {
                    vid.load();
                }
            });
            ...
            vid.addEventListener('progress', () => {
                // Aggressive buffer check - hide spinner quickly
                if (vid.buffered.length > 0) {
                    const bufferedEnd = vid.buffered.end(vid.buffered.length - 1);
                    const duration = vid.duration;
                    if (duration > 0) {
                        const bufferedPercent = (bufferedEnd / duration) * 100;
                        // Hide spinner as soon as we have any reasonable buffer
                        if (bufferedPercent > 5 || bufferedEnd > 3) {
                            buffer.classList.remove('show');
                        }
                    }
                }
            ...
            vid.addEventListener('canplay', () => {
                buffer.classList.remove('show');
                console.log('Can play - ready state:', vid.readyState);
            });
            
            function toggle() {
                if (vid.paused) {
                    // Check if we have enough buffer before playing
                    const canPlayNow = vid.readyState >= 3 || (vid.buffered.length > 0 && vid.buffered.end(0) > vid.currentTime + 2);
                    ...
                    if (canPlayNow) {
                        ...
                        // Not enough buffer, show spinner
                        buffer.classList.add('show');
                        vid.load();
            ...
            vid.onwaiting = () => {
                buffer.classList.add('show');
                console.log('Buffering... Ready state:', vid.readyState);
                
                // Try to recover from buffering quickly
                if (vid.readyState < 3) {
                    // Force reload to get more data
            ...
            vid.oncanplaythrough = () => {
                buffer.classList.remove('show');
                console.log('Can play through - ready state:', vid.readyState);
            };
            
            // Better buffering detection
            vid.onstalled = () => {
                buffer.classList.add('show');
                console.log('Stalled - ready state:', vid.readyState);
            ...
            vid.onsuspend = () => {
                // Don't show buffer on suspend, it's normal
                console.log('Suspend - ready state:', vid.readyState);
            };
            vid.onplaying = () => {
                buffer.classList.remove('show');
                console.log('Playing - ready state:', vid.readyState);
            };
            vid.onloadeddata = () => {
                buffer.classList.remove('show');
                console.log('Data loaded - ready state:', vid.readyState);
            };
            
            // Handle seeking with smart buffering
            vid.onseeking = () => {
                buffer.classList.add('show');
                console.log('Seeking to:', vid.currentTime);
            };
            vid.onseeked = () => {
                // Only hide buffer if we have data at new position
                if (vid.readyState >= 3) {
                    buffer.classList.remove('show');
                }
                console.log('Seeked to:', vid.currentTime, 'Ready state:', vid.readyState);
            };
